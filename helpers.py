...
# input: features and label stored in the csv file
# output: X_train, Y_train
def get_data_from_csv(csv_file, sep, n_features=-1, header='infer'):
    data = pd.read_csv(csv_file, sep=sep, header=header)  # import dataframe
    if n_features == -1:
        X_data = data.iloc[:, :-1].values # split data into features and labels
        Y_data= data.iloc[:, -1].values
    else:
        X_data = data.iloc[:, :n_features].values # split data into features and labels
        Y_data= data.iloc[:, n_features:].values
